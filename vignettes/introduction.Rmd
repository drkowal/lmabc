...
Consider the model from the previous section. Now, let's use `lmABC`.

```{r}
fit <- lm_abc(y ~ age + race + age:race, df)
summary(fit)
```

In this new model output, `age` corresponds to the global effect, and the other slopes modify the global effect. For instance, being in the NHW category is associated with an expected change of `r round(coef(fit)[["raceNHW"]], 3)` units in $y$ compared to the population as a whole, all else equal.
