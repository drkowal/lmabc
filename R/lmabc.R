...
lmabc <- function(formula, data, ..., cprobs = NULL) {
	...
	if (any(is.na(coef(fit0)))) {
		stop('NAs found in the OLS estimators; consider a different formula statement')
	}
	...
	if (is.null(Con)) {
		...
		cov.unscaled_con <- crossprod(t(Qm),
																	tcrossprod(summary(fit_con)$cov.unscaled,
																						 Qm))
		colnames(cov.unscaled_con) <- xnames
		rownames(cov.unscaled_con) <- xnames

		# Check fitted values to make sure the fits are identical:
		tol <- 10^-8  # tolerance
		if (mean((stats::fitted(fit_con) - stats::fitted(fit0))^2) + mean((X%*%beta_con - stats::fitted(fit0))^2) > tol) {
			stop('Fitted values are not the same; may be an issue with the constraint matrix')
		}
...
getConstraints <- function(formula, data, cprobs = NULL) {
	...
	if (length(f_inds) > 0) {
		...
		cnames <- names(data)[f_inds]
		names(cdat) <- names(data)[f_inds]  # correct the names
		...
		for (k in 1:length(c_inds)) {  # for each categorical variable
			...
			for (ell in 1:length(covar[c_inds[[k]]])) {  # for each appearance
				...
				names_kl <- sapply(paste0(cnames[k], names(pi_hat[[k]])),
		...
		if (length(inds_catcat) > 0) {
			...
			for (j in inds_catcat) {
				...
				Con1 <- array(0,
											c(length(pi_hat[[v2]]), p),
											dimnames = list(paste0(v1, ":",v2, names(pi_hat[[v2]])), xnames))
				for (k in 1:length(pi_hat[[v2]])) {
					# Update the names to match the model.matrix labeling
					names_kl <- paste0(v1, names(pi_hat[[v1]]),
														 ":",
				...
				Con2 <- array(0,
											c(length(pi_hat[[v1]]), p),
											dimnames = list(paste0(v1, names(pi_hat[[v1]]),":",v2), xnames))
				for (k in 1:length(pi_hat[[v1]])) {
					# Update the names to match the model.matrix labeling
					names_kl <- paste0(v1, names(pi_hat[[v1]])[k],
														 ":",
														 v2, names(pi_hat[[v2]]))
					# Add the constraint at the right indices:
					inds_kl <- match(names_kl, xnames)
...
getFullDesign <- function(formula, data, center = TRUE) {
	...
	if (length(f_inds) > 0) {
		...
		X <- stats::model.matrix(object = formula,
														 data = data,
														 contrasts.arg = lapply(cdat, stats::contrasts, contrasts=FALSE))
	} else {
